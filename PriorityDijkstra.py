...
def dijkstra(g, rad, arr):
    ...
    tree = [
        rad]                                                #Inizializzo un array per i nodi visitati, chiamato "tree" con dentro "rad"
        ...
        for citta in tree:                                  #per ogni città inserita nell'albero (all'inizio solo rad)
            if citta in g.streets:                          #esisterà un insieme di archi per cui posso uscire da quelle città
                for strade in g.streets[citta]:
                    #print strade.peso
                    newcosto = strade.peso + costo[
                        strade.par]      #il costo per arrivare alle città collegate sarà pari al costo dell'arco
                    # più quello speso per arrivare al nodo appena precedente
                    if newcosto < costo[
                        strade.arr]:                #se questo nuovo costo è minore del costo che avrei speso per arrivare a quella città da un altro percorso
    ...
    return costo[
        arr]                                               #ritorno il costo della città d'arrivo (il migliore finora trovato)
...
def check(g):
    ...
    for i in range(len(g.streets)):             #per ogni lista di strade uscenti dal nodo i-esimo
        if g.streets[
                    i + 1] == []:                #se queste sono state tutte percorse (quindi rimosse dalla funzione "dijkstra")
